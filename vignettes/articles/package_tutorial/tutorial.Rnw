...
A different block approach, related to the iterated block particle filter for spatiotemporal POMP models \citep{ning23,ionides22}, is implemented using the \code{block=TRUE} argument to \code{panelPomp::mif2}.
Recent experiments suggestion this approach justifies further consideration.
...
The deterministic search also benefits from a block optimization strategy, alternating between shared and unit-specific parameters, as used for the stochastic search \citep{breto19}.
